...
package org.thor.base.utils.take.luban;
...
import static org.thor.base.utils.take.luban.Preconditions.checkNotNull;
...
public class Luban {
    ...
    // for single file
    public Disposable launch(final OnCompressListener listener) {
    ...
    // for multi file
    public Disposable launch(final OnMultiCompressListener listener) {
        checkNotNull(listener, "the listener cannot be null !");
        ...
        List<Flowable<File>> observables = new ArrayList<>();
        for (File file : mFileList) {
            observables.add(Flowable.just(file).map(new Function<File, File>() {
                @Override
                public File apply(File file) throws Exception {
                    return compressImage(gear, file);
                }
        ...
        return Flowable.zip(observables, new Function<Object[], List<File>>() {
            @Override
    ...
    public Flowable<File> asObservable(Scheduler scheduler) {
        return asObservableCurrentThread().subscribeOn(scheduler);
    }

    public Flowable<File> asObservableCurrentThread() {
        checkNotNull(mFile,
                "the image file cannot be null, please call .load() before this method!");
    ...
    public Flowable<List<File>> asListObservable() {
        return asListObservable(Schedulers.computation());
    }

    public Flowable<List<File>> asListObservable(Scheduler scheduler) {
        return asListObservableCurrentThread().subscribeOn(scheduler);
    }

    public Flowable<List<File>> asListObservableCurrentThread() {
